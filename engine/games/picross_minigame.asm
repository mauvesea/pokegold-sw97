; Ported from pret/pokegold-spaceworld, revision
; 5b555967e18171d06c6c7f630c0fcc9eeba52519. See docs/picross.md.
...
; load cursor GFX
	...
	ld bc, 6 tiles ; cursor + dust; do not read beyond the asset
	ld a, BANK(PicrossCursorGFX)
	call FarCopyBytes

; TO BE CHECKED: use the project's Chris sprite in place of prototype Gold.
; load player sprites
	ld de, vTiles0 + $100
	ld hl, ChrisSpriteGFX + LEN_2BPP_TILE * 4 ; Gold's back-facing standing sprite
	ld bc, 4 tiles
	ld a, BANK(ChrisSpriteGFX)
	call FarCopyBytes

	ld de, vTiles0 + $140
	ld hl, ChrisSpriteGFX + LEN_2BPP_TILE * 16 ; Gold's back-facing walking sprite
	ld bc, 4 tiles
	ld a, BANK(ChrisSpriteGFX)
	call FarCopyBytes
	...
	ld [hli], a
	ld [hl], $10 ; player tiles begin at vTiles0 tile $10
	ret
...
.InitRAM:
	ld hl, wPicrossRAM
	ld bc, wPicrossRAMEnd - wPicrossRAM
	xor a
	call ByteFill
	ret
...
; The NPC supplies a checked, zero-based stage index.
	ld a, [wScriptVar]
...
PicrossSprites:
	...
	picross_pattern PATTERN_NORMAL, GhostIcon
	picross_pattern PATTERN_NORMAL, UnownIcon
	picross_pattern PATTERN_NORMAL, SnorlaxIcon
; The identical tiles exist here in a compressed tileset. Include just the
; original top/bottom pairs contiguously to avoid decompression scratch.
	picross_pattern PATTERN_NORMAL, PicrossPokecenterPattern
	picross_pattern PATTERN_NORMAL, PoliwrathSpriteGFX
	assert (@ - PicrossSprites) / 4 == NUM_PICROSS_STAGES

PicrossNumbersGFX:
INCBIN "gfx/picross/picross_numbers.2bpp"

PicrossBackgroundGFX:
INCBIN "gfx/picross/picross_background.2bpp"

PicrossGridHighlightsGFX:
INCBIN "gfx/picross/picross_highlights.2bpp"

PicrossGridGFX:
INCBIN "gfx/picross/picross_grid.2bpp"

PicrossCursorGFX:
INCBIN "gfx/picross/picross_cursor.2bpp"

; Preserve the prototype's interrupt-sampled pressed edges. GetJoypad in this
; project instead derives edges between calls, losing the cursor's forced edge.
Picross_GetJoypad:
	ldh a, [hJoypadPressed]
	ldh [hJoyPressed], a
	ldh a, [hJoypadDown]
	ldh [hJoyDown], a
	ldh [hJoyLast], a ; hInMenu is set throughout Picross
	ldh a, [hJoyPressed]
	and a
	jr z, .repeat
	ld a, 15
...
; The prototype transfers eight tiles per VBlank. Keep its two-frame grid
; reads/writes even though this project's general helper transfers more tiles.
Picross_Request2bpp:
	ldh a, [hBGMapMode]
	push af
	xor a
...
PicrossPokecenterPattern:
INCBIN "gfx/tilesets/pokecenter.2bpp", $20 tiles, 2 tiles
INCBIN "gfx/tilesets/pokecenter.2bpp", $30 tiles, 2 tiles
